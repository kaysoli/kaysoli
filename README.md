...
- **Records Terminal & Lookups**: Use the new `RecordsDatabase`/`RecordsManager` plus `RecordsTerminalPanel` to load subject/vehicle/officer data from assets or CSV text sheets. Voice commands like "run plate 7XKZ991" trigger `LookupRecord` intent, log the response, and play the requesting unit's assigned voice.
